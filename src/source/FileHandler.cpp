...
Candidate FileHandler::deserializeCandidate(const std::string& data) {
    ...
    if (fields.getSize() >= 3) {
        Candidate candidate(fields.get(0), fields.get(1), fields.get(2));
        
        // Set vote count if available
        if (fields.getSize() >= 4 && !fields.get(3).empty()) {
            try {
                int voteCount = std::stoi(fields.get(3));
                // We need a way to set vote count - might need to add this method to Candidate
...
bool FileHandler::saveAllData(const ElectionSystem& system) {
    ...
    // Access internal data via getters
